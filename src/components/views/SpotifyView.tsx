...
export const SpotifyView: React.FC<SpotifyViewProps> = ({
  ...
  // View options customization popover & settings
  ...
  useEffect(() => {
    setVisibleCount(50);
  }, [baseStreams, filterArtist, q, viewOptions.filterPreset, viewOptions.sortBy, viewScope]);

  // IntersectionObserver for progressive infinite scrolling
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        if (entries[0].isIntersecting) {
          setVisibleCount((prev) => Math.min(prev + 50, displayStreams.length));
        }
      },
  ...
  // Lazily sliced streams to keep DOM lightweight
  const visibleStreams = useMemo(() => {
    return displayStreams.slice(0, visibleCount);
  }, [displayStreams, visibleCount]);
  ...
  const renderCustomSpotifyCard = (item: TimelineItem) => {
    ...
    if (viewOptions.layoutMode === 'grid') {
      return (
        <div
          key={item.id}
          onClick={() => handleSelectCard(item)}
          className={`flex flex-col bg-white dark:bg-[#121212] rounded-2xl overflow-hidden cursor-pointer transition-all border group ${
            isSelected
              ? 'border-emerald-500 ring-2 ring-emerald-500/30 bg-emerald-500/5 dark:bg-emerald-500/10 shadow-sm'
              : 'border-gray-200 dark:border-gray-800 hover:border-emerald-500/40 hover:shadow-xs'
          } ${viewOptions.density === 'compact' ? 'p-2.5' : viewOptions.density === 'spacious' ? 'p-4' : 'p-3'}`}
        >
          {viewOptions.showAlbumArt && (
            <div className="relative aspect-video rounded-xl overflow-hidden mb-2.5 shrink-0 border border-gray-100 dark:border-gray-800/80 flex flex-col justify-between p-3 shadow-inner group">
              <SpotifyCoverArt
                title={item.title}
                artist={item.subtitle}
                album={item.album}
    ...
    return (
      <div
        key={item.id}
        onClick={() => handleSelectCard(item)}
        className={`bg-white dark:bg-[#121212] rounded-2xl cursor-pointer transition-all border group ${
          isSelected
            ? 'border-emerald-500 ring-2 ring-emerald-500/30 bg-emerald-500/5 dark:bg-emerald-500/10 shadow-sm'
            : 'border-gray-200 dark:border-gray-800 hover:border-emerald-500/40 hover:shadow-xs'
      ...
      >
        ...
        <div className="flex gap-3 items-start">
          {viewOptions.showAlbumArt && (
            <div className="w-20 h-16 rounded-xl overflow-hidden relative shrink-0 border border-gray-100 dark:border-gray-800 shadow-inner group-hover:scale-102 transition-transform">
              <SpotifyCoverArt
                title={item.title}
                artist={item.subtitle}
                album={item.album}
  ...
  if (spotifyItemsAll.length === 0) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center bg-white dark:bg-black rounded-2xl p-8 text-center border border-gray-200 dark:border-gray-800">
        <Headphones className="w-12 h-12 text-emerald-500 mb-4 opacity-50" />
        ...
        <button
          ...
          className="bg-emerald-500 text-white px-4 py-2 rounded-xl text-xs font-semibold shadow-sm hover:bg-emerald-600 transition-colors flex items-center gap-1.5 cursor-pointer"
        >
  ...
  return (
    <div className="flex-1 flex flex-col bg-white dark:bg-black rounded-2xl shadow-sm border border-gray-200 dark:border-gray-800 overflow-hidden min-h-0 relative">
      ...
      {/* Top Reusable ViewToolbar (Exact YouTube Pattern) */}
      <ViewToolbar
        title="Spotify Music"
        ...
        searchPlaceholder="Search tracks, artists, albums, or #tags..."
        searchResultsCount={q ? displayStreams.length : undefined}
        onImportClick={onImportClick}
        importLabel="Import"
      >
        {/* All controls sit directly in front in single top bar - Zero Clutter! */}
        <div className="flex items-center gap-1.5 flex-wrap">
          {/* Day vs All Scope Toggle */}
          <div className="flex bg-gray-200/80 dark:bg-gray-900 p-0.5 rounded-xl text-xs font-semibold">
            <button
              ...
              title="Show only tracks from currently selected day"
            >
              Day
            </button>
            <button
              ...
              title="Search and view all Spotify history"
            >
              All
            </button>
          </div>

          {/* Subview & Layout Switcher (Combined) */}
          <div className="flex bg-gray-200/80 dark:bg-gray-900 p-0.5 rounded-xl text-xs">
            <button
              onClick={() => {
                setSubView('day');
                updateViewOption('layoutMode', 'feed');
              }}
              className={`px-2 py-1 font-semibold rounded-lg flex items-center gap-1 transition-all cursor-pointer ${
                subView === 'day' && viewOptions.layoutMode === 'feed'
            ...
            <button
              onClick={() => {
                setSubView('day');
                updateViewOption('layoutMode', 'grid');
              }}
              className={`px-2 py-1 font-semibold rounded-lg flex items-center gap-1 transition-all cursor-pointer ${
                subView === 'day' && viewOptions.layoutMode === 'grid'
                  ? 'bg-white dark:bg-gray-800 shadow-xs text-gray-900 dark:text-white'
                  : 'text-gray-500 hover:text-gray-700 dark:text-gray-400'
              }`}
              title="Grid View"
            ...
            <button
              onClick={() => {
                setSubView('day');
                updateViewOption('layoutMode', 'compact');
              }}
              className={`px-2 py-1 font-semibold rounded-lg flex items-center gap-1 transition-all cursor-pointer ${
                subView === 'day' && viewOptions.layoutMode === 'compact'
                  ? 'bg-white dark:bg-gray-800 shadow-xs text-gray-900 dark:text-white'
                  : 'text-gray-500 hover:text-gray-700 dark:text-gray-400'
              }`}
          ...
          <div className="flex items-center gap-1 bg-gray-100 dark:bg-gray-900 px-2 py-1 rounded-xl border border-gray-200 dark:border-gray-800 text-xs shadow-2xs">
            ...
            <select
              ...
              onChange={(e) => updateViewOption('sortBy', e.target.value as SortOption)}
          ...
          <div className="relative">
            <button
              ...
              title="Display Options (Album Art, Timestamps, Grouping)"
            >
              ...
              <div className="absolute right-0 top-full mt-2 w-72 bg-white dark:bg-[#181818] border border-gray-200 dark:border-gray-700 rounded-2xl shadow-xl p-3.5 space-y-3 z-50 animate-in fade-in zoom-in-95">
                ...
                <div className="space-y-1">
                  ...
                  <select
                    ...
                    onChange={(e) => updateViewOption('groupBy', e.target.value as GroupOption)}
                    className="w-full bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl px-2 py-1 text-xs text-gray-700 dark:text-gray-300 outline-none cursor-pointer"
                  >
                    <option value="none">None (Chronological)</option>
                    <option value="time_of_day">Time of Day (Morning/Afternoon/Night)</option>
                ...
                <div className="space-y-2 pt-1 border-t border-gray-100 dark:border-gray-800">
                  ...
                  <div className="space-y-1.5 text-xs">
                    <label className="flex items-center gap-2 cursor-pointer select-none">
                      <input
                        type="checkbox"
                        checked={viewOptions.showAlbumArt}
                        onChange={(e) => updateViewOption('showAlbumArt', e.target.checked)}
                        className="rounded text-emerald-500 focus:ring-emerald-500"
                      />
                      <span className="text-gray-700 dark:text-gray-300">Album Art / Record Sleeve</span>
                    </label>

                    <label className="flex items-center gap-2 cursor-pointer select-none">
                      <input
                        type="checkbox"
                        checked={viewOptions.showArtistBadge}
                        onChange={(e) => updateViewOption('showArtistBadge', e.target.checked)}
                        className="rounded text-emerald-500 focus:ring-emerald-500"
                      />
                      <span className="text-gray-700 dark:text-gray-300">Artist Name</span>
                    </label>

                    <label className="flex items-center gap-2 cursor-pointer select-none">
                      <input
                        type="checkbox"
                        checked={viewOptions.showAlbumName}
                        onChange={(e) => updateViewOption('showAlbumName', e.target.checked)}
                        className="rounded text-emerald-500 focus:ring-emerald-500"
                      />
                      <span className="text-gray-700 dark:text-gray-300">Album Tag</span>
                    </label>

                    <label className="flex items-center gap-2 cursor-pointer select-none">
                      <input
                        type="checkbox"
                        checked={viewOptions.showDuration}
                        onChange={(e) => updateViewOption('showDuration', e.target.checked)}
                        className="rounded text-emerald-500 focus:ring-emerald-500"
                      />
                      <span className="text-gray-700 dark:text-gray-300">Track Duration</span>
                    </label>
                    ...
                    <label className="flex items-center gap-2 cursor-pointer select-none">
                      <input
                        type="checkbox"
                        checked={viewOptions.showPlayCountBadge}
                        onChange={(e) => updateViewOption('showPlayCountBadge', e.target.checked)}
                        className="rounded text-emerald-500 focus:ring-emerald-500"
                      />
                      <span className="text-gray-700 dark:text-gray-300">Repeat Stream Badge</span>
                    </label>
                  </div>
                </div>
                ...
                <div className="pt-2 border-t border-gray-100 dark:border-gray-800 flex justify-between items-center">
                  <button
                    ...
                    className="text-[11px] text-gray-500 hover:text-emerald-500 font-medium cursor-pointer"
                  >
                    Reset Defaults
                  </button>
                  <button
                    onClick={() => setIsViewOptionsOpen(false)}
                    className="px-2.5 py-1 bg-emerald-500 text-white rounded-lg text-xs font-semibold hover:bg-emerald-600 transition-colors cursor-pointer"
                  >
                    Done
                  </button>
                </div>
      ...
      <div className="flex-1 flex flex-col lg:flex-row min-h-0 overflow-hidden">
        ...
        <div
          className={`flex flex-col min-h-0 overflow-hidden transition-all ${
            isRightPanelOpen
              ? 'w-full lg:w-3/5 xl:w-7/12 border-b lg:border-b-0 lg:border-r border-gray-200 dark:border-gray-800'
              : 'w-full'
          }`}
        >
          {/* Subview Contents */}
              ...
              <>
                {displayStreams.length === 0 ? (
                  ...
                  groupedStreams.map(group => (
                    <div key={group.groupName || 'all-items'} className="space-y-2.5">
                      {group.groupName && (
                      ...
                      <div
                        className={
                          viewOptions.layoutMode === 'grid'
                            ? 'grid grid-cols-1 sm:grid-cols-2 gap-3'
                            : 'space-y-2.5'
                        }
                      >
                        {group.items.map(item => renderCustomSpotifyCard(item))}
                      </div>
                    </div>
                  ))
                )}
                ...
                {visibleCount < displayStreams.length && (
                  <div ref={observerTarget} className="py-4 text-center">
                    <button
                      onClick={() => setVisibleCount(prev => Math.min(prev + 50, displayStreams.length))}
                      className="px-4 py-2 text-xs font-semibold text-gray-500 dark:text-gray-400 hover:text-emerald-500 dark:hover:text-emerald-400 bg-gray-100 dark:bg-gray-850 hover:bg-gray-200 dark:hover:bg-gray-800 rounded-xl transition-all cursor-pointer inline-flex items-center gap-2 shadow-2xs"
                    >
                      <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
                      <span>Loading more tracks... ({Math.min(visibleCount, displayStreams.length)} of {displayStreams.length.toLocaleString()})</span>
                    </button>
                  </div>
                )}
              </>
              ...
              (() => {
                ...
                return (
                  <div className="flex-1 flex flex-col overflow-hidden min-h-0">
                    ...
                    <div className="flex-1 grid grid-cols-7 overflow-y-auto bg-white dark:bg-black min-h-0">
                  ...
                  <div className="flex-1 flex flex-col overflow-hidden min-h-0">
                    ...
                    <div className="flex-1 overflow-y-auto min-h-0">
                      <table className="w-full text-left border-collapse">
                        <thead className="bg-gray-50/90 dark:bg-gray-900/90 sticky top-0 z-10 border-b border-gray-200 dark:border-gray-800 text-[10px] font-bold text-gray-400 uppercase tracking-wider">
                          <tr>
                      ...
                      {visibleCount < data.length && (
                        <div className="py-4 text-center">
                          ...
                          >
                            <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
                            <span>Loading more logs... ({Math.min(visibleCount, data.length)} of {data.length.toLocaleString()})</span>
                          </button>
                        </div>
                      )}
                    </div>
        ...
        {isRightPanelOpen && selectedTrackItem && selectedTrackMetrics && (
          <div className="w-full lg:w-2/5 xl:w-5/12 bg-gray-50/50 dark:bg-[#0d0d0d] flex flex-col min-h-0 overflow-y-auto">
            {/* Header with Eyebrow, Title, Actions */}
            <div className="p-4 sm:p-5 border-b border-gray-200 dark:border-gray-800 bg-white dark:bg-[#121212] sticky top-0 z-20 space-y-3">
            ...
            <div className="p-4 sm:p-5 space-y-5">
              ...
              {activePanelTab === 'analytics' && (
                <div className="space-y-5">
                  ...
                  <div className="relative aspect-video rounded-2xl overflow-hidden shadow-md border border-gray-200 dark:border-gray-800 group p-4 flex flex-col justify-between">
                    <SpotifyCoverArt
                  ...
                  <div className="grid grid-cols-2 gap-2.5 text-xs">
                    <div className="p-3 rounded-2xl bg-white dark:bg-[#141414] border border-gray-200 dark:border-gray-800 shadow-2xs">
                  ...
                  <div className="p-4 rounded-2xl bg-white dark:bg-[#141414] border border-gray-200 dark:border-gray-800 space-y-2.5">
                    <span className="text-[10px] font-bold uppercase tracking-wider text-gray-400 block">
