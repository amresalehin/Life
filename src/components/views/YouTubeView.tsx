...
export const YouTubeView: React.FC<YouTubeViewProps> = ({
  ...
  useEffect(() => {
    setVisibleCount(50);
  }, [baseVideos, filterChannel, q, viewOptions.filterPreset, viewOptions.sortBy, viewScope]);
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        if (entries[0].isIntersecting) {
          setVisibleCount((prev) => Math.min(prev + 50, displayVideos.length));
        }
      },
  ...
  // Lazily sliced videos to keep DOM lightweight
  const visibleVideos = useMemo(() => {
    return displayVideos.slice(0, visibleCount);
  }, [displayVideos, visibleCount]);
  ...
  const subNavButtons = (
    <div className="flex bg-gray-200/80 dark:bg-gray-900 p-0.5 sm:p-1 rounded-xl">
      <button
  ...
  if (ytItemsAll.length === 0) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center bg-white dark:bg-black rounded-2xl p-8 text-center border border-gray-200 dark:border-gray-800">
        <Youtube className="w-12 h-12 text-red-500 mb-4 opacity-50" />
        ...
        <button
          ...
          className="bg-red-500 text-white px-4 py-2 rounded-xl text-xs font-semibold shadow-sm hover:bg-red-600 transition-colors flex items-center gap-1.5 cursor-pointer"
        >
  ...
  const renderCustomVideoCard = (item: TimelineItem) => {
    ...
    if (viewOptions.layoutMode === 'grid') {
      return (
        <div
          key={item.id}
          onClick={() => handleSelectCard(item)}
          className={`flex flex-col bg-white dark:bg-[#121212] rounded-2xl overflow-hidden cursor-pointer transition-all border ${
            isSelected
              ? 'border-red-500 ring-2 ring-red-500/30 bg-red-500/5 dark:bg-red-500/10 shadow-sm'
              : 'border-gray-200 dark:border-gray-800 hover:border-red-500/40 hover:shadow-xs'
    ...
    return (
      <div
        key={item.id}
        onClick={() => handleSelectCard(item)}
        className={`bg-white dark:bg-[#121212] rounded-2xl cursor-pointer transition-all border ${
          isSelected
            ? 'border-red-500 ring-2 ring-red-500/30 bg-red-500/5 dark:bg-red-500/10 shadow-sm'
            : 'border-gray-200 dark:border-gray-800 hover:border-red-500/40 hover:shadow-xs'
  ...
  return (
    <div className="flex-1 flex flex-col bg-white dark:bg-black rounded-2xl shadow-sm border border-gray-200 dark:border-gray-800 overflow-hidden min-h-0 relative">
      ...
      <ViewToolbar
        ...
        searchPlaceholder="Search videos, channels, tags..."
      ...
      >
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {/* Unified View Mode Switcher (Feed, Grid, Compact, Week, Month, Table) */}
          <div className="flex bg-gray-100 dark:bg-gray-900 p-0.5 rounded-xl border border-gray-200 dark:border-gray-800 text-xs">
            <button
              onClick={() => {
                setSubView('day');
          ...
          {/* Channel Filter Dropdown (In Front) */}
          <div className="flex items-center gap-1 bg-gray-100 dark:bg-gray-900 px-2 py-1 rounded-xl border border-gray-200 dark:border-gray-800 text-xs shadow-2xs">
          ...
          <div className="relative">
            <button
              ...
              title="Display Options (Thumbnails, Timestamps, Grouping)"
            >
              ...
              <div className="absolute right-0 top-full mt-2 w-72 bg-white dark:bg-[#181818] border border-gray-200 dark:border-gray-700 rounded-2xl shadow-xl p-3.5 space-y-3 z-50 animate-in fade-in zoom-in-95">
                ...
                <div className="space-y-1">
                  ...
                  >
                    ...
                    <option value="channel">Channel / Creator</option>
                  </select>
                </div>
                ...
                <div className="space-y-2 pt-1 border-t border-gray-100 dark:border-gray-800">
                  ...
                  <div className="space-y-1.5 text-xs">
                    <label className="flex items-center gap-2 cursor-pointer select-none">
                      <input
                        type="checkbox"
                        checked={viewOptions.showThumbnails}
                        onChange={(e) => updateViewOption('showThumbnails', e.target.checked)}
                        className="rounded text-red-500 focus:ring-red-500"
                      />
                      <span className="text-gray-700 dark:text-gray-300">Video Thumbnails</span>
                    </label>

                    <label className="flex items-center gap-2 cursor-pointer select-none">
                      <input
                        type="checkbox"
                        checked={viewOptions.showChannelBadge}
                        onChange={(e) => updateViewOption('showChannelBadge', e.target.checked)}
                        className="rounded text-red-500 focus:ring-red-500"
                      />
                    ...
                    <label className="flex items-center gap-2 cursor-pointer select-none">
                      <input
                        type="checkbox"
                        checked={viewOptions.showWatchCountBadge}
                        onChange={(e) => updateViewOption('showWatchCountBadge', e.target.checked)}
                        className="rounded text-red-500 focus:ring-red-500"
                      />
                      <span className="text-gray-700 dark:text-gray-300">Repeat Watch Badge</span>
                    </label>
                  </div>
                </div>
                ...
                <div className="pt-2 border-t border-gray-100 dark:border-gray-800 flex justify-between items-center">
                  <button
                    ...
                    className="text-[11px] text-gray-500 hover:text-red-500 font-medium cursor-pointer"
                  >
                    Reset Defaults
                  </button>
                  <button
                    onClick={() => setIsViewOptionsOpen(false)}
                    className="px-2.5 py-1 bg-red-500 text-white rounded-lg text-xs font-semibold hover:bg-red-600 transition-colors cursor-pointer"
                  >
                    Done
                  </button>
                </div>
      ...
      <div className="flex-1 flex flex-col lg:flex-row min-h-0 overflow-hidden">
                ...
                {displayVideos.length === 0 ? (
                  ...
                  groupedSections.map(section => (
                    <div key={section.key} className="space-y-2.5">
                      ...
                      >
                        {section.items.map(item => renderCustomVideoCard(item))}
                      </div>
                    </div>
                  ))
                )}
                ...
                {visibleCount < displayVideos.length && (
                  <div ref={observerTarget} className="py-4 text-center">
                    ...
                    >
                      ...
                      <span>Loading more videos... ({Math.min(visibleCount, displayVideos.length)} of {displayVideos.length.toLocaleString()})</span>
                    </button>
                  </div>
                )}
              </>
        ...
        {isRightPanelOpen && activeVideoAnalytics && (
          <div className="w-full lg:w-2/5 xl:w-5/12 flex flex-col min-h-0 overflow-y-auto bg-gray-50/50 dark:bg-[#0c0c0c] border-t lg:border-t-0">
